...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.new_turtle_subscriber_ = self.create_subscription(NewTurtlePos, "new_turtle_pos", self.cb_add_turtle_to_list, 10)
        self.control_loop_timer_ = self.create_timer(0.01, self.control_loop)
        self.get_logger().info("Turtle Controller Running...")
        
    def cb_turtle_pose(self, msg):
        self.pose_ = msg
    def cb_add_turtle_to_list(self, msg):
        self.turtles_.append(msg)
    
    def catch_turtle(self):
        # call service to kill turtle / return bool
        # log
        self.get_logger().info(f'{self.turtles_[0].turtle_name} has been caught')
        # remove first turt el
        del self.turtles_[0]
        # reset target x,y
        self.x_target = self.turtles_[0].turtle_x
        self.y_target = self.turtles_[0].turtle_y
